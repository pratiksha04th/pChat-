...
class AllUsersController extends BaseController {
  ...
  AllUsersController({
    ...
    DatabaseReference? usersRef,
        ...
        usersRef = usersRef ?? FirebaseDatabase.instance.ref("users");
  ...
  void onInit() {
    ...
    auth.authStateChanges().listen(_handleAuthState);
  ...
  void _attachRealtimeListeners() {
    _addSub = usersRef.onChildAdded.listen(_upsertUser);

    _changeSub = usersRef.onChildChanged.listen(_upsertUser);

    _removeSub = usersRef.onChildRemoved.listen((event) {
  ...
  Future<void> syncEmailWithDatabase() async {
    ...
    await usersRef.child("users/${user.uid}").update({
      "email": latestEmail,
  ...
  Future<void> saveFcmToken() async {
      ...
      await usersRef.child("${user.uid}/fcmToken").set(token);
  }
